...
- `pnpm check` はハーネスの整合性、lint、knip、型検査、Vitest、Web/API/runner の build、Storybook の build とブラウザテストを実行する。Chromium の初回準備は `pnpm --filter @life-console/web exec playwright install chromium`。
- `pnpm knip` は全 workspace の未使用ファイル・依存関係・export を検査する。[knip.jsonc](knip.jsonc) の entry・除外を変更するときは、実際の起動経路や参照元を確認し、理由をコメントに残す。
