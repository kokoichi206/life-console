...
- 列が取り得る文字列は `packages/domain/src/index.ts` の名前付き候補定義を正本にし、Drizzle の `text(..., { enum: 候補定義 })` と Zod で共有する。DB schema から API の入出力 DTO を参照して型を合わせない。SQLite のこの enum 指定は型推論用で、DB の CHECK 制約や実行時の検証は追加しない。
