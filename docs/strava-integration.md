...
- 活動ごとの消費 kcal だけを D1 の `strava_activity_calories` に永続保存し、カロリー収支に使う。保存する項目は活動 ID・開始日時・状態（`pending` / `measured` / `unavailable`）・消費 kcal・登録日時・一覧で最後に見た日時・取得日時に限る。**この永続保存は [Strava API Policy](https://www.strava.com/legal/api_policy) の 6.2（7 日を超える保持の禁止）と 5.5（後から取り出せる形での保存の禁止）に反する。6.3 の 48 時間以内の削除反映も、本人が画面を開いたときだけ同期する方式では保証できない。本人専用・外部非公開のダッシュボードであることと保存項目を最小限にとどめることを前提に、ユーザーが 2026-09-13 に規約の原文を確認した上で選択した。本書は規約適合を主張しない。**
- 認証情報は D1 の `strava_connection` に AES-GCM で暗号化して保持する。更新・接続・解除は同じ lease で直列化し、期限切れや古い処理の保存を拒否する。
- `運動を更新` と画面表示で一覧を再取得し、未取得の活動があれば `strava_calories_sync` ジョブを予約する。runner が API 経由で詳細を順に取得し、消費 kcal を保存する。webhook は使わない。GPS コースは `View on Strava` から確認する。
- 消費 kcal の取得は API がレート制限を制御する。応答の `X-ReadRateLimit-Limit` / `X-ReadRateLimit-Usage` と `X-RateLimit-Limit` / `X-RateLimit-Usage`（[Rate Limits](https://developers.strava.com/docs/rate-limits/)）から使用率を読み、15 分枠の閾値に達したら次の境界まで待ち、日次の閾値に達したらそのジョブを打ち切る。画面表示の一覧取得も同じ予算を使うため、閾値は上限より手前に置く。取得待ちのまま残った行は次の表示か `運動を更新` で再開する。
- Strava 側で削除された活動は、ジョブの reconcile が消す。reconcile は一覧をページ単位で読み（Worker の 1 リクエストあたりの subrequest 上限に全ページが収まらないため）、現れた活動の「最後に見た日時」を進める。空ページに達した時点で、期間内の行のうちこのジョブの開始時刻より前にしか見ていない行を削除する。一覧の取得に失敗したときは削除も登録も行わない。詳細取得で 404 が返った活動も消す。画面表示側の登録は削除を行わない（期間全体を見終えたことをページ単位の 1 リクエストでは判定できない）。表示していない期間の削除は反映されない。
- `接続を解除` は Strava の revoke API が成功した後に、保存した認証情報・画面の運動データ・保存済みの消費 kcal をすべて消す。失敗は表示し、再試行できるよう接続情報を保持する。再接続後は消費 kcal を取り直す。
