...
体重の推移の下に「カロリー収支」を置き、`基準消費量 + 運動 − 摂取` を日別の棒で示す。運動には Strava の活動ごとの実測消費 kcal を使う。未接続なら運動を含めない収支を出し、その旨を注記する。取得途中・失敗のときは収支を出さない。
...
- 活動一覧はブラウザのメモリ内に置き、画面・期間の切り替えや接続解除で破棄する。取得結果を HTTP キャッシュに残さない。名前・種目・距離・時間・心拍は保存しない。
