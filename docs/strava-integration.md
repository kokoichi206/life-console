...
本人用の読み取り連携。健康画面で選んだ期間のランニング、ウォーキング、筋トレなどを取得し、週の走行距離・回数・移動時間、平均体重、食事件数を並べる。週を選ぶと、体重・運動・食事写真やメモを同じ期間に絞る。日本時間・月曜始まりで集計し、期間の端の週は「一部」と表示する。
...
2026-09-09、規約の適用に未確定点があることを確認した上で、本人用 API を試す方針になった。条文の存在と個別用途への解釈を分けた調査は [設計レビュー](reviews/grok-strava-design-review-2026-09-09.md) に記録する。本実装を Strava による個別許可取得済みとは扱わない。
