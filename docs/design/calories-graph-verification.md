...
| 既定の表示範囲（2.1 節） | `recentBalanceWindow`（または実装の同等の関数）と `HealthPage` での使い方、そのテスト | 既定は終端から 7 暦日（`from = max(periodFrom, periodTo − 6 日)`）。期間セレクターの値に関係なく 7 日。隠れる日数 `n` は期間全体の暦日数 − 7 で、0 以下なら絞らない。テストに期間が 7 日ちょうど・8 日・7 日未満の境界がある | 8 日や 6 日になっている。特定の期間だけ 7 日になる。`n` の計算がずれている |
| 切り替えボタン（2.1 節） | `DailyCalorieBalanceList` の表の下のボタン | 折りたたみ時「すべて表示（他 n 日）」、展開時「直近 7 日だけ表示」。期間が 7 日以下なら描かない。`aria-expanded` か同等の状態を持つ | 文言が違う。7 日以下でも出る。押しても行数が変わらない |
| 展開状態の URL（2.1 節） | `health-search.ts`、`HealthRoutePage.tsx` | 展開のキーが `parseHealthSearch` にあり、既定（7 日）では URL に載らない。不正な値は `search.strict` で除かれる。期間変更の navigate で引き継がれ、再読み込みで復元される | 既定でも載る。期間ボタンで展開が戻る。`useState` だけで持っていて再読み込みで消える |
| 取得範囲の不変（2.1 節） | `HealthPage` の `mealsForPeriodQuery` / `stravaCaloriesQuery` / `useStravaActivities` の引数と `pendingActivities` の計算元 | 引数は `periodFrom` / `periodTo` のままで、絞り込みは `calorieBalanceRows` に渡す `from` だけ。`pendingActivities` は期間全体の件数 | 折りたたみ時に取得の from も 7 日に縮む。取得待ち件数が 7 日分になる |
| 表示範囲とスケール（2.1 節） | `scaleKcal` の計算元 | 表示している行から計算する（展開で変わりうる）。期間全体から計算する実装でもよいが、折りたたみと展開で一貫している | 同じ日の棒の長さが切り替えで不規則に変わる |
| 空白期間の端（2.1 節） | 既定の 7 日の先頭が記録なしの日のときの行 | 単独の記録なしの日は 1 日分の行（摂取 `—`、棒なし、副行「食事の記録なし」）になり、期間全体の空白期間は展開後に「9/2〜9/7 食事と運動の記録なし（6 日）」の 1 行になる | 既定表示に期間全体の空白期間の行が出る（絞れていない）。先頭の記録なしの日が消える |
...
| --- | --- | --- | --- |
| 1 | 部品「貯金と超過の日がある」 | 行の構成 | ヘッダーが「日付」「摂取」「0」「運動」「収支」で、`−500 / +500` の目盛りがない。新しい日が上。9/13 は棒トラックの左端に「1,703」、右向きの棒、右端に「+320」、収支「+117」で副行なし。9/10 は「1,902」、左向きの赤い棒、「+180」、収支「超過 −222」で副行なし。9/8 は「1,900」「+500」「+100」（`1,500 + 500 − 1,900`）。9/9 は摂取「—」、棒なし、右端「+410」、収支「—」、副行「食事の記録なし」。既定では 9/13〜9/7 の 7 行だけで、9/7 は摂取「—」・棒なし・副行「食事の記録なし」の行。表の下に「すべて表示（他 5 日）」（12 日 − 7）。story が展開状態で描く場合は 9/2〜9/7 が「食事と運動の記録なし（6 日）」の 1 行になり、ボタンは「直近 7 日だけ表示」。行内に「摂取」「運動」の語が出ない。中央のゼロ線が全行でつながって見える。凡例があり、カロリー収支内には「Powered by Strava」がない |
| 1b | 同上（該当する展開の story があればそれ） | 表示範囲の切り替え | 「すべて表示（他 5 日）」を押すと 9/2〜9/7 の 1 行が現れ、文言が「直近 7 日だけ表示」に変わる。もう一度押すと 7 行に戻る。押しても既存の行の数値は変わらない |
| 1c | 期間が 7 日以下の story（あれば） | ボタンの有無 | ボタンが出ず、全行が出る |
...
| 14 | ページ「摂取と運動を同じ期間で見る」 | 統合 | 既定で直近 7 日分の行だけが出て、収支の行に「+100」「超過」が見える（サンプルの日付が 7 日の外なら展開してから確認する）。「すべて表示（他 n 日）」で全行が出て URL に展開のキーが付く。期間ボタン（30 日 / 直近 90 日）で行が変わり、展開状態は維持される |
...
| 11 | 期間ボタンで 30 日 → 直近 90 日 → 全期間 | 既定ではどの期間でも今日から 7 日分の行だけが出る。表の下のボタンが「すべて表示（他 n 日）」で、`n` は期間の暦日数 − 7（30 日なら 23、直近 90 日なら 83）。URL の `range` / `from` / `to` が変わり、再読み込みで復元される |
| 11a | 直近 90 日で「すべて表示（他 83 日）」を押す | 期間全体の行が出て、食事も運動もない連続日は「記録なし（n 日）」の 1 行にまとまる。文言が「直近 7 日だけ表示」に変わり、URL に展開のキーが付く。既存の 7 行の数値は変わらない |
| 11b | 展開のまま再読み込み → 「直近 7 日だけ表示」を押す | 再読み込み後も展開のまま。戻すと 7 行になり、URL からキーが消える |
| 11c | 展開したまま「30 日」を押す | 30 日分の行が出て展開が維持される（ボタンは「直近 7 日だけ表示」のまま） |
| 11d | 「日付を指定」で終了日の 4 日前を開始日にする（5 日間） | ボタンが出ず、5 日分の行が出る |
| 11e | devtools の Network で切り替え前後の `/api/v1/meals?from&to` を比べる | `from` / `to` は期間全体のまま変わらない。切り替えで新しい取得が走らない |
...
| 2 | 直後の「カロリー収支」パネル | 期間内の活動がある日の行の副行に「取得待ち n 件」が付き、パネルに「消費カロリーを取得中（残り n 件）」。この `n` は期間全体の件数で、既定の 7 日表示のままでも「すべて表示」で展開しても同じ値。負の収支の行は灰色の破線（取得待ちがあるので超過を確定しない）。正で摂取全件の行は通常色の破線 |
