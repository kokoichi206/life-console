...
| 定期実行 | 今回は登録しない。payload に期間が要るため既存のスケジュール API（`payload: {}`）には乗らない |
...
- 表示していない期間の削除は反映されない。全期間の同期はユーザーが「全期間」を表示したときに行われる。
...
- `strava_calories_sync` の定期スケジュール登録、webhook、自動リトライ。
- `unavailable` の活動の再確認。
