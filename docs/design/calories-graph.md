...
| 一覧の取得完了 | `complete` | 日ごとの実測値。取得待ち・算入外があれば件数。期間内に取得待ちがある間はパネルに `role="status"` で「消費カロリーを取得中（残り n 件）。Mac の runner が順に取得します」。`n` は期間全体の件数で、表示を直近 7 日に絞っていても変わらない（2.1 節） | 描く。カロリー収支内には「Powered by Strava」を表示しない |
...
消費カロリーを取得中（残り 1 件）。Mac の runner が順に取得します。
 ...
 █ 貯金  ▓ 超過  ░ 未確定  │ ±0
 収支 = 基準消費量 + 運動 − 摂取。運動は Strava の記録の消費カロリーです。
```
