...
- Strava から本人の運動を読み取り、既定は体重のみを表示する。「走行距離を重ねる」で必要なときだけ週の走行距離の棒を重ねて、体重の変化と運動量を同じ時間軸で振り返る。グラフで確認した週の運動と食事を開ける。回数・移動時間などの週次集計は折り畳んだ表で確認する。Garmin から同期した活動と Strava で直接記録した活動を同じ取得経路で扱う。心拍は取得できた値のみ表示する。睡眠と練習強度の詳細分析は将来の検討対象。活動一覧は表示期間ごとに取得し、保存しない。活動ごとの消費 kcal だけを D1 へ永続保存し、runner のジョブが Strava のレート制限内で順に取得する。取得は 2 時間ごとの定期同期で直近 30 日を突き合わせ、初回だけ下限日まで 90 日ずつ遡る。画面表示と『運動を更新』も即時の補助として同じジョブを予約する。この永続保存は Strava API Policy 6.2・5.5・6.3 に反しており、2026-09-13 に本人が規約の原文を確認した上で選択した。保存・認可・レート制限の仕様は [Strava 連携](strava-integration.md) を参照。
...
| Strava の消費 kcal | 2 時間ごとに直近 30 日。初回は下限日まで 90 日ずつ遡る。画面表示・運動の更新時にも予約 |
| 金融取引 | 1 日ごと |
| 資産残高 | 1 週間ごと、または手動 |
| ローカルリポジトリ一覧 | 1 日ごと |
