...
- 食事は写真とメモを中心に記録する。カロリーは初回登録時と詳細画面で任意に手入力できる。栄養素の手入力は不要。
- 手入力カロリーは一覧・詳細・日別合計に優先して反映し、0 kcal も入力済みとして扱う。解析済みの栄養素は推定値として表示する。手入力済みでも個別解析・再解析でき、解析中の手入力でジョブを中止しない。解析結果は手入力カロリーを上書きしない。初回から手入力した食事は自動予約・一括解析の対象外とする。
- 写真なしの記録も、メモがあれば詳細から個別解析・再解析できる。メモだけの記録は自動予約・一括解析の対象外とする。食品や分量が判断できなければ失敗を表示し、既存の推定値は保持する。
- 手入力カロリーがない写真付きの食事を保存すると runner の解析ジョブを自動予約し、推定カロリーと栄養素を後から追記する。
- 記録した事実と agent の推定結果は分けて保存する。

### 1.4 ダッシュボードとモバイル

- 全画面で 640 px 未満のページ左右余白は 16 px とする。セクションの外枠カードは見出しと区切り線に置き換え、本文の左右余白を重ねない。個別の選択・編集・送信確認、食事タイル、入力欄の枠は残す。640 px 以上はカード表示を維持する。
...
- メインナビゲーションに『やること』を追加し、仕事・私生活のタスクと買い物を扱う。
- タスクと買い物は保存構造・入力を分ける。仕事タスクを別のタスクとして複製しない。
- タスクの実施日時と期日は区別し、どちらも任意にする。参照リンクとメモを付けられるようにする。
- 買い物は場所を継続利用し、その場所の欄から品名だけで追加する。追加時の場所入力やセレクター選択は求めない。
- 買うものと場所は多対多。既存品を別の場所にも紐づけられるようにする。
- 購入状態は品ごとに共有する。スマホでは行を右にスワイプして離すと未購入一覧から外す。短い移動・逆方向・縦スクロール・操作中断では変更しない。チェックボックスは置かず、PC では購入ボタン、編集欄では端末を問わず購入状態を変更できる。直後の『元に戻す』と購入済み一覧から取り消せるようにする。
...
| 食事の入力事実 | D1 と R2 |
| 食事の栄養推定 | D1 の派生レコード |
| Strava の活動ごとの消費 kcal | D1（Strava からの取得値） |
| 1 日の基準消費量 | D1（本人が設定した単一の値） |
...
現在の機能は [README](../README.md#仕事)、認証の前提は [運用上の注意](operations.md#外部サービスとファイル連携) を参照する。Talknote の実データ検証は本人セッションの再認証待ち。
...
2026-09-09 追加: ローカルで進む agent の作業を Life Console で管理し、本人は判断が必要な点と人に送る文章を最終確認する。

- 最初の実装は、既存のタスクから起動した agent が質問を提出し、ホームで回答・修正指示を返す経路とする。
- 質問と回答は job の実行世代に紐づけて保存する。本人の未回答がある間だけ `waiting_for_user` にし、通常の agent 実行と区別する。
- 回答は一度だけ確定できる。終了・中止要求・lease 失効後の実行へ回答を渡さない。
- 入力中の回答はタブ内に保持し、再読み込みでも復元する。回答の保存後に消去する。
- 質問への回答は外部サービスへの送信操作ではない。送信文の専用確認画面、Project からの自動起動、任意のタイミングでの指示追加は後続の実装範囲。
...
- 2026-09-18 追加。健康ページだけをリンク保有者へ読み取り専用で共有する。ログインは不要で、相手も期間・表示条件を変更できる。記録・編集・同期・解析・CSV 取り込みは非活性とし、共有 API も更新を受け付けない。本人画面と共通の画面・読み取り処理を使い、共有対象は健康ページの体重・目標・運動・食事・写真・栄養・カロリー収支。仕事・家計・運用状況は共有しない。

- 実測値と 7 日移動平均を表示する。画面幅が狭くてもグラフの高さと文字サイズを維持する。
- 1 本指の横ドラッグで表示期間を移動し、2 本指で拡大縮小・移動できる。タップで記録を表示し、長押し後の横移動では期間を固定して各日の数値を追う。縦スワイプはページをスクロールする。
- 数値はグラフ上部の固定欄に表示し、指を離しても読める。『最新へ』で表示日数を保って最新の記録へ戻れる。補助の移動ボタンと日付入力でも調整でき、操作終了時の期間を URL で復元する。
- 開始体重、目標体重、任意の期限を設定・保存・解除できる。最新の記録を基準に目標への進捗を表示し、グラフに目標体重の水平線を引く。
- スマホでは健康の指標を 2 列、食事・体重の記録ボタンを 1 行にまとめる。
- 健康画面は 640 px 未満では左右の余白を 16 px とし、各セクションをカードで囲まず見出しと区切り線で分ける。個別に開く食事のタイルと入力欄は枠を残し、640 px 以上ではセクションのカード表示を維持する。
- Obsidian 同期では、DB にある日は D1、DB にない過去分は既存 CSV を正とする。
- CSV の手動取込に対応する。
- runner から自動取込できるようにする。
- 手入力にも対応する。体脂肪率（%）は体重と同じ計測日時の任意項目として保存する。未入力は欠測として扱う。
- 体重には `occurred_at` と `recorded_at` を持たせる。
- Strava から本人の運動を読み取り、既定は体重のみを表示する。追加表示は『消費カロリー』『走行距離』『体脂肪率』から 1 つを選ぶ。選択中のボタンを再度押すと解除し、体重のみの表示に戻す。消費カロリーと走行距離は週の棒、体脂肪率は % の独立軸の線で表示し、体脂肪率未記録の測定点では線をつながない。体重の変化と運動量を同じ時間軸で振り返る。グラフで確認した週の運動と食事を開ける。回数・移動時間などの週次集計は折り畳んだ表で確認する。Garmin から同期した活動と Strava で直接記録した活動を同じ取得経路で扱う。心拍は取得できた値のみ表示する。睡眠と練習強度の詳細分析は将来の検討対象。運動名・種目・開始日時・距離・移動時間・経過時間・平均心拍数と活動ごとの消費 kcal を D1 へ保存し、runner のジョブが Strava のレート制限内で取得・更新する。運動一覧とカロリー収支は D1 の保存済みデータを表示する。取得は 2 時間ごとの定期同期で直近 30 日を突き合わせ、初回だけ下限日まで 90 日ずつ遡る。画面表示では Strava アクセスもジョブ予約も行わない。『運動を同期』で表示期間の同期を明示的に依頼できる。2026-09-14 に保存範囲と画面の読み取り元を変更する指示を受けた。この永続保存は Strava API Policy 6.2・5.5・6.3 に反しており、2026-09-13 に本人が規約の原文を確認した上で選択した。保存・認可・レート制限の仕様は [Strava 連携](strava-integration.md) を参照。
- 2026-09-19 追加。週の消費カロリーは保存済みの全種目の運動分を集計する。未取得・取得不可を合計に足さず、取得済み分と件数を表示する。全件未取得は欠損として表示し、今週・期間端には対象日数を添える。基準消費量は含めない。表示名は『消費カロリー』とし、推定値・保存済みのみである説明を残す。
- Obsidian への同期では、runner が D1 の体重・体脂肪率を既存の weight-trend.csv に日付でマージする。体脂肪率は末尾の body_fat_percent 列に保存し、未記録は空欄とする。グラフ用 JS の bf にも反映する。D1 にない過去分は削除せず、アーカイブを含めたグラフ用データを再生成する。日本時間で最後の測定を日別の代表値とし、同日の CSV と D1 の値が異なれば D1 を採用する。実行頻度と相対ディレクトリは DB、vault の実パスは Mac の非公開設定に置く。候補と制約は [同期仕様](weight-obsidian-export.md) を参照。

2026-09-16 に Simple アプリの利用終了に伴い、Android の画面操作による体重取り込みと専用スキルを廃止した。現在の Obsidian 同期は Life Console の D1 から runner 経由で行い、Android 端末への接続は不要。

### 6.7 食事

- 写真、メモ、発生日時、記録日時、食事区分、任意の tag を記録する。
- 写真とメモだけで登録を完了できる。
- 端末に保存済みの写真を選ぶ操作と、カメラで撮影する操作を分ける。選択した写真を確認し、登録前に左へ 90° ずつ回転、選び直し・取り消しができる。プレビューと同じ向きで保存する。
- 食事記録は写真を並べた一覧で見返し、写真・日時・メモの詳細を開ける。
- 写真は client 側で縮小し、EXIF を除去する。
- 短命な署名付き URL を使い、R2 へ直接アップロードする。
- 入力した事実と agent の推定結果を別レコードにする。
- 手入力カロリーがない写真付きの食事の保存時に解析ジョブを自動予約し、Mac の runner が次回ポーリングで写真とメモを解析して推定カロリーと栄養素を追記する。保存と予約は同時に確定し、再送では重複予約しない。画像解析の既定は Codex CLI の `gpt-5.6-luna`、推論は `low` とする。画像解析用の環境変数で Codex CLI / Claude CLI / Gemini CLI とモデルを切り替えられる。食事詳細から個別解析・再解析し、未解析写真をまとめて処理することもできる。
- 食事ごとの手入力値または推定値と、日本時間の日別合計を表示する。カロリー未記録の件数を明示し、値がない食事を 0 kcal と扱わない。
- 2026-09-13 追加。1 日の基準消費量を本人が 1 つの数値として設定・解除できる。基礎代謝は身長・年齢・性別から計算せず、履歴も持たない。
- 2026-09-13 追加。日別のカロリー収支を `基準消費量 + 運動 − 摂取` で表示する。体重グラフと同じ期間・日本時間・新しい順で、ゼロを中央に置いた棒で貯金と超過を示す。運動は Strava の実測値だけを使い、MET などの概算に落とさない。
- 2026-09-13 追加。取得待ちの運動を 0 kcal と扱わない。未記録の食事は摂取を増やす方向、取得待ちの運動は収支を上げる方向にしか動かないため、符号と額の確定を別々に判定して未確定を明示する。消費 kcal を取得できない活動は「算入外」として合計から除き、件数を示す。基準消費量が未設定のとき、運動の一覧を取得中・取得失敗のときは収支を出さない。
...
- 『同期・実行状況』に会話同期の定期実行管理を置く。未設定のサービスも表示し、登録状態・次回実行・直近の結果を確認できるようにする。
- Gmail・Slack・Chatwork・Talknote の毎時・2 時間ごと・毎日・毎週の登録、頻度変更、一時停止・再開、今すぐ同期を扱う。登録時に初回を即時予約するか選択できる。アカウントと取得範囲は既存の runner 設定を使う。
- 停止は今後の予約だけを止め、既存 job の中止とは分ける。頻度変更・再開時は現在時刻から新しい間隔後に次回を設定し、停止期間の実行をまとめて予約しない。
- 二重登録と古い画面からの設定上書きを拒否する。会話同期以外の定期実行と、返信送信・agent 起動の自動化はこの管理画面の対象外。
...
| 体重 | 手動または日和見実行 |
| 食事の登録 | 即時 |
| 食事の栄養解析 | 写真付き食事の保存時に予約。runner の次回ポーリングで実行 |
| Strava の消費 kcal | 2 時間ごとに直近 30 日。初回は下限日まで 90 日ずつ遡る。『運動を同期』で明示的に予約。画面表示は DB 読み取りだけ |
| 金融取引 | 1 日ごと |
| 資産残高 | 1 週間ごと、または手動 |
| ローカルリポジトリ一覧 | 1 日ごと |
...
PWA の追加機能、複数リポジトリ UI、高度な provider 切り替え、Android native は MVP 後に実装する。Talknote と返信下書きは 2026-09-06 の追加要件として実装対象に含める。
