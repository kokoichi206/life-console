...
- Strava から本人の運動を読み取り、既定は体重のみを表示する。「走行距離を重ねる」で必要なときだけ週の走行距離の棒を重ねて、体重の変化と運動量を同じ時間軸で振り返る。グラフで確認した週の運動と食事を開ける。回数・移動時間などの週次集計は折り畳んだ表で確認する。Garmin から同期した活動と Strava で直接記録した活動を同じ取得経路で扱う。心拍は取得できた値のみ表示する。睡眠と練習強度の詳細分析は将来の検討対象。初回実装は表示期間ごとの取得とし、保存・認可の仕様は [Strava 連携](strava-integration.md) を参照。
