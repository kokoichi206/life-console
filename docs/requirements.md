...
- 写真付きの食事を保存すると runner の解析ジョブを自動予約し、推定カロリーと栄養素を後から追記する。
...
- 写真付きの食事の保存時に解析ジョブを自動予約し、Mac の runner が次回ポーリングで写真とメモを解析して推定カロリーと栄養素を追記する。保存と予約は同時に確定し、再送では重複予約しない。画像解析の既定は Codex CLI の `gpt-5.6-luna`、推論は `low` とする。画像解析用の環境変数で Codex CLI / Claude CLI / Gemini CLI とモデルを切り替えられる。食事詳細から個別解析・再解析し、未解析写真をまとめて処理することもできる。
- 食事ごとの推定値と、日本時間の日別合計を表示する。未解析の件数を明示し、推定値がない食事を 0 kcal と扱わない。
