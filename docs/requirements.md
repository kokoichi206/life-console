...
- 手入力カロリーは一覧・詳細・日別合計に優先して反映し、0 kcal も入力済みとして扱う。手入力済みの食事は自動予約・一括解析・再解析の対象外とし、処理中の解析結果で上書きしない。詳細で手入力した場合は、既存の個別解析予約の中止と値の保存を同時に確定する。実行中は中止を要求し、中止待ちの状態を表示する。一括解析では手入力と競合した結果だけを保存せず、残りを続ける。
