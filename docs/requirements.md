...
- Strava から本人の運動を読み取り、週の走行距離・回数・移動時間と体重・食事を同じ期間で振り返る。Garmin から同期した活動と Strava で直接記録した活動を同じ取得経路で扱う。心拍は取得できた値のみ表示する。睡眠と練習強度の詳細分析は将来の検討対象。初回実装は表示期間ごとの取得とし、保存・認可の仕様は [Strava 連携](strava-integration.md) を参照。
