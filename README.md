...
| 対象 | 記録・振り返り |
| --- | --- |
| 体重 | 体重と任意の体脂肪率を記録。実測値と 7 日移動平均、目標への進捗。グラフには走行距離または体脂肪率を追加表示し、選択中のボタンを再度押すと解除。過去の CSV 取り込みと Obsidian への書き出し |
...
運動一覧とカロリー収支は DB の保存済みデータを表示し、画面表示では Strava にアクセスしません。定期同期の結果を反映し、『運動を同期』から表示期間の同期を依頼できます。未同期の運動は含まれないことを注記し、初回同期前は運動ゼロとみなしません。
