...
[deploy](.github/workflows/deploy.yml) は `develop` への push、または Actions 画面の『Run workflow』で実行します。Markdown、`docs/`、`.agents/`、`.claude/` だけの変更では自動実行を省略します。手動実行でも `develop` を選択してください。他のブランチではデプロイ job を実行しません。
