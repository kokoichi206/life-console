...
| 運動 | [Strava の本人用連携](docs/strava-integration.md)。週の走行距離・回数・時間を体重・食事と同じ期間で振り返る。活動ごとの消費カロリーを Strava から取得して保存し、日別の収支に使う |
...
カロリー収支は保存済みの運動カロリーを先に表示し、Strava の活動一覧を待ちません。最新情報の確認中・失敗時は保存済みの値であることを示し、保存が空の間は運動ゼロとみなしません。
