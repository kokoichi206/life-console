...
健康画面では目標の開始体重・目標体重・任意の期限を設定し、DB に保存します。最新の体重から達成率を表示し、グラフには目標体重の水平線を引きます。`/health?entry=goal` で設定を開き、編集・解除できます。

グラフは画面幅に追従し、高さと文字サイズを維持します。30 日・90 日・年別・全期間の切り替え、日付指定、拡大縮小・移動ボタンに対応します。タッチ画面では 1 本指の横ドラッグで期間を移動し、2 本指のピンチで拡大縮小できます。タップした記録は指を離しても上部に表示し、長押ししてなぞると期間を固定したまま各日の数値を確認できます。縦スワイプはページをスクロールします。『最新へ』で表示日数を保って最新の記録へ戻れます。日付入力と移動ボタンは『日付を指定・ボタンで移動』から使えます。ドラッグ中は滑らかに描画し、操作終了時に日付単位で表示期間を確定します。表示期間は `range` または `from` / `to` の URL パラメーターに保存し、再読み込みや入力シートの開閉でも保持します。スマホでは指標を 2 列、食事・体重の記録ボタンを 1 行に表示します。
