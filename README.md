...
| 対象 | 記録・振り返り |
| --- | --- |
| 体重 | 体重と任意の体脂肪率を記録。実測値と 7 日移動平均、目標への進捗。グラフには週の消費カロリー・走行距離・体脂肪率を切り替えて追加表示し、選択中のボタンを再度押すと解除。過去の CSV 取り込みと Obsidian への書き出し |
| 運動 | [Strava の本人用連携](docs/strava-integration.md)。週の走行距離・回数・時間を体重・食事と同じ期間で振り返る。運動名・距離・時間・心拍数・消費カロリーを定期同期で保存し、運動一覧と日別収支は DB から表示する |
| 食事 | 写真とメモ、任意のカロリー入力による記録。未入力なら runner で推定し、食事ごとと日別の合計を表示 |
| 家計 | 月ごとの収支、カテゴリ別・支払手段別の支出、資産配分と純資産の推移。手入力と CSV 取り込み |

健康ページは、リンクを知っている人に読み取り専用で共有できます。本人画面の『共有リンク』で現在の表示条件を渡し、受け取った人も期間・表示条件を変更できます。記録・編集・同期・解析・CSV 取り込みのボタンは非活性になります。[共有の設定と停止方法](docs/operations.md#健康ページの共有)を参照してください。
...
『同期・実行状況』の『連絡の定期実行』で、Gmail・Slack・Chatwork・Talknote の未設定・有効・停止中、次回実行、直近の同期結果を確認できます。1 時間・2 時間・1 日・1 週間ごとの登録、頻度変更、一時停止・再開、今すぐ同期に対応します。初回を登録直後に実行するかも選べます。取得元と範囲は Mac の既存設定を使い、返信送信や agent 起動は予約しません。
...
運動一覧とカロリー収支は DB の保存済みデータを表示し、画面表示では Strava にアクセスしません。定期同期の結果を反映し、『運動を同期』から表示期間の同期を依頼できます。未同期の運動は含まれないことを注記し、初回同期前は運動ゼロとみなしません。体重グラフの『消費カロリー』は、全種目の取得済みカロリーを日本時間・月曜始まりで集計します。未取得・取得不可の件数を併記し、全件未取得の週は欠損として表示します。今週や期間端の週には対象日数を示します。基準消費量は含めず、週を選ぶと同じ期間の運動と食事を確認できます。

初回の食事登録でカロリーを手入力できます。一覧から開く詳細画面でも変更できます。手入力したカロリーは一覧・詳細・日別合計で優先し、解析済みのたんぱく質・脂質・炭水化物も推定値として表示します。手入力後も個別解析・再解析でき、解析中に手入力してもジョブは継続します。解析結果は手入力カロリーを上書きしません。初回から手入力した食事は、自動予約・一括解析の対象外です。
...
推定値は元の記録と別の既存 `nutrition_estimates` テーブルに保存し、使用モデル、解析日時、画像とメモの入力 hash を記録します。再解析の履歴を残し、手入力がなければ、一覧と日別集計には最新の結果だけを使います。日別合計は日本時間で画面の選択期間内の全記録を集計し、カロリー未記録を 0 kcal として扱わず、手入力・解析済みの記録件数を表示します。手入力の 0 kcal は記録済みです。写真がない記録も、メモがあれば詳細の『栄養を解析』『栄養を再解析』から推定できます。メモだけの記録は自動予約・一括解析の対象外です。食品や分量を判断できない場合は失敗として表示し、以前の解析結果を保持します。

既存のスケジュール API で `jobKind: "nutrition_analysis"`、`interval: "daily"`、`payload: {}` を登録すると、未解析写真を定期処理できます。この変更だけでは本番のスケジュールは追加しません。

消費カロリーの同期も同じスケジュール API で `jobKind: "strava_calories_sync"`、`interval: "every_2_hours"`、`coalescing: "skip_if_pending"`、`deadlineSeconds: 7200`、`payload: {}` を登録します。2 時間ごとに直近 30 日を同期し、初回だけ下限日まで 90 日ずつ遡ります。この変更だけでは本番のスケジュールは追加しません。
