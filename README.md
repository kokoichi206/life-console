...
グラフは画面幅に追従し、高さと文字サイズを維持します。30 日・90 日・年別・全期間の切り替え、日付指定、拡大縮小・移動ボタンに対応します。タッチ画面では横方向の 2 本指操作で拡大縮小と期間移動ができます。表示期間は `range` または `from` / `to` の URL パラメーターに保存し、再読み込みや入力シートの開閉でも保持します。スマホでは指標を 2 列、食事・体重の記録ボタンを 1 行に表示します。
