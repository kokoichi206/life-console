...
Drizzle schema、D1 migration、ローカル確認用の架空 seed を管理します。実行時の業務クエリと Result への変換は `apps/api/src/repositories` に置きます。API からこのパッケージの schema を参照し、読み書き・集計・条件付き保存に Drizzle のクエリビルダーを使います。JSON や日付の関数などは `sql` 式を併用します。
