...
体重の入力画面を開く、ボタン 1 個の Android ウィジェットです。Android 12 以上に対応します。

## 使い方

1. APK をインストールし、Life Console Widget を開きます。
2. 『ホーム画面に追加』を押して、ランチャーの追加画面で確定します。
3. ウィジェットの『体重を記録』をタップします。

自動追加に対応しないランチャーでは、ホーム画面を長押しし、ウィジェット一覧から『体重を記録』を追加します。アプリにもこの手順を表示します。Oshi Launcher では『ホームに追加』→『ウィジェット』→『Life Console Widget』→『体重を記録』です。

起動先は `https://life-console.kokoichi206.workers.dev/health?entry=weight` です。URL を扱うブラウザまたはインストール済み Web アプリで開きます。Cloudflare Access のセッションが切れていればログインが必要です。アプリ内の『記録画面を開く』からも同じ URL を開けます。
...
- `WeightEntry.kt` が起動 URL、`weight_widget.xml` がウィジェットの見た目を定義。
