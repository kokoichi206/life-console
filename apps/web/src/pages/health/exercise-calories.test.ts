...
const measured = (activityId: string, caloriesKcal: number): StravaActivityCalories => ({ activityId, occurredAt: "2026-09-07T00:00:00Z", status: "measured", caloriesKcal });
...
describe("日別の運動消費カロリー", () => {
  ...
  it("保存行のない活動と pending を取得待ちに、unavailable を算入外に数え、どちらも合計に入れない", () => {
    const byDay = exerciseCaloriesByDay(
      ...
      [measured("stored", 320), { activityId: "waiting", occurredAt: "2026-09-07T01:00:00Z", status: "pending", caloriesKcal: null }, { activityId: "manual", occurredAt: "2026-09-07T03:00:00Z", status: "unavailable", caloriesKcal: null }],
    );
